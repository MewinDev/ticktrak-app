...
<div x-data="{
    ...
    x-on:close-modal.window="show = $event.detail === '{{ $name }}' ? false : show" x-on:close.stop="show = false"
    x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="!$event.shiftKey && nextFocusable().focus()"
    x-on:keydown.shift.tab.prevent="prevFocusable().focus()" x-show="show"
    class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0 mt-10" aria-labelledby="modal-title-{{ $name }}"
    aria-modal="true" role="dialog" style="display: none;">
    <!-- Backdrop -->
    <div x-show="show" class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity"
        x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        @if ($closeOnClickAway) x-on:click="show = false" @endif></div>
    ...
    <div x-show="show"
        ...
        <!-- Modal Header -->
        <div class="@if (!$header) hidden @else flex @endif items-center justify-between px-4 py-3 border-b border-gray-300 dark:border-gray-700">
